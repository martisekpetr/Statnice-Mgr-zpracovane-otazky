...
Predikát (nebo relace) $\R \subseteq \N^n, n \geq 1$ je \textbf{primitivně} (resp. \textbf{obecně}) \textbf{rekurzivní predikát (PRP, ORP)}, pokud je jeho charakteristická funkce primitivně (resp. obecně)
rekurzivní. Obecně rekurzivním predikátům a relacím budeme též říkat \textbf{rekurzivní}.

Predikát (nebo relace) $\R \subseteq \N^n, n \geq 1$ je \textbf{rekurzivně spočetný (RSP)}, pokud existuje funkce $n$ proměnných $f_R$, pro kterou platí, že
...
Hodnota funkce v tomto případě není důležitá. Takové funkci budeme také říkat \textit{částečná charakteristická funkce.}

Unární rekurzivní (resp. rekurzivně spočetný) predikát $A \subseteq N$ budeme též nazývat \textbf{rekurzivní množinou} (resp. \textbf{rekurzivně spočetnou množinou}).
...
\begin{enumerate}
	...
	\begin{align*}
		2r'(x) - r(x) - r'(g) &= \log_2\left(\frac{s'(x)}{s(x)}\right) + \log_2\left(\frac{s'(x)}{s'(g)}\right)\\
							  ...
							  & = 1 + 1 = 2
	\end{align*}
	Druhá nerovnost je dána díky $s'(x) \geq s(x) + s'(g)$ (viz obrázek) a protože díky vlastnostem logaritmu je nejnižší možná hodnota dosažena právě při volbě $s(x) = s'(g) = \frac{s'(x)}{2}$ %TODO zigzig_logaritmus
...
%TODO splay - statická optimalita, entropie
...
\begin{itemize}
	...
	\item \textbf{transpozice matice $n \times n$:} 
		...
		\begin{enumerate}
			...
			\end{figure}
		\end{enumerate}
		\item \textbf{násobení matic}
		\item \textbf{merge-sort}
		\item \textbf{funnel-sort}
\end{itemize}
...
%TODO další cache oblivious struktury, merge a funnel sort
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Function{ART1}{}
		\State $\rho \gets$ hodnota z intervalu $[0,1]$ \Comment{práh bdělosti}
		\State $t = 0$
		\For{$i$ \textbf{in} 0 \textbf{to} $N-1$}
			\For{$j$ \textbf{in} 0 \textbf{to} $M-1$}
				\State $t_{ij}(0) \gets 1$ \Comment{váha mezi vstupním neuronem $i$ a výstupním neuronem $j$ v čase 0}
				\State $b_{ij}(0) \gets \frac{1}{1+N}$ \Comment{váha mezi výstupním neuronem $j$ a vstupním neuronem $i$ v čase 0}
			\EndFor		
		\EndFor
		\While{existuje vstupní vzor}
			\State $\vec{x} \gets$ vstupní vzor 
			\For{j \textbf{in} 0 \textbf{to} $M-1$}
				\State $\mu_j = \sum_{i=0}^{N-1} b_{ij}(t) x_{i}$ \Comment{výstup výstupního neuronu $j$}
			\EndFor
			\State $k = \arg\max_j \{\mu_j\}$ \Comment{neuron nejlépe odpovídající vstupnímu vzoru}
