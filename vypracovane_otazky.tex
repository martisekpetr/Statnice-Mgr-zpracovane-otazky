...
Stačí tedy zvolit $c_L = (\log_2 |\Sigma| + \log_2 |Q| + 1)$. 
\end{proof}
...
v nejvýše $i$ krocích Turingova stroje $M$.
\bigskip

\begin{minipage}{\linewidth}
	\begin{lstlisting}[language=Python, frame=single, escapeinside={(*}{*)}]
(*\textit{Dosažitelná}($i, K_1, K_2$)*):
  if i = 0:
	  if (*$(K_1, K_2)$*) in E or (*$K_1 == K_2$*):
		  return true
	  else
		  return false
  for K in V:
	  if (*\textit{Dosažitelná}($\left \lceil \frac{i}{2} \right \rceil, K_1, K$)*) and (*\textit{Dosažitelná}($\left \lceil \frac{i}{2} \right \rceil, K, K_2$)*):
		  return true
  return false
	\end{lstlisting}
\end{minipage}

Víme, že počet vrcholů je nejvýše $2^{c_Mf(n)}$ a tedy voláním \textit{Dosažitelná}$(2^{c_M f(n)}, K_0^x, K_F^x)$ na grafu $G_{M,x}$ zjistíme, zda existuje přijímací výpočet.
...
Hloubka rekurze je omezena pomocí $\log_2(2^{c_M f(n)}) = c_Mf(n) = O(f(n))$, protože počáteční hodnota $i$ je $2^{c_M f(n)}$ a v každém dalším voláním toto $i$ snížíme na polovinu. 
...
kde $C^{-1}$ je množina bootstrap výběrů neobsahujících vzorek $i$.
...
\begin{itemize}
  ...
  \item dimension reduction (PCA, PCR, PLS)
\end{itemize}
...
\begin{itemize}
	\item definujeme \textbf{residuum} $i$-tého vzorku jako $e_i = y_i - \hat{y}_i = y_i - (\hat\beta_0 + \hat\beta_1x_i)$
...
\subsubsection{Lineární metody pro klasifikaci (logistická regrese, LDA, QDA)}
  \begin{itemize}
	...
	\item LINEÁRNÍ metody = hranice mezi třídami je lineární
	...
	\subparagraph{Vícerozměrná logistická regrese}
	Více prediktorů:
	$$p(X) = \frac{\exp(\beta_0 + \beta_1X_1 + \dots + \beta_pX_p)}{1 + \exp(\beta_0 + \beta_1 X_1 + \dots + \beta_pX_p)}$$
	...
	\begin{itemize}
		...
		\item různá cena chyby $\to$ posun hranice z 0.5 třeba na 0.2
		\item ROC křivka
	\end{itemize}
...
\begin{itemize}
	\item \textit{kořene a vnitřních vrcholů označených atributem}, ze kterého vede jedna hrana pro každou možnou hodnotu tohoto atributu (případně interval pro spojité predikáty)
...
\begin{itemize}
	...
	\item \textbf{klasifikační} -- list obsahuje nejčetnější třídu v regionu daném atributy. Jako míru chyby lze použít:
	\begin{itemize}
	...
	\begin{algorithmic}[1]
		\Function{ID3}{$data,G,X$}
			...
			\For{$x_j$ \textbf{in} $X_j$}
				...
				\State $data_{X_j = x_j} \gets$ podmnožina $data$, kde $X_j = x_j$
				\If{$data_{X_j = x_j} = \emptyset$}
					\State přidej list označený nejčastější $g$ v data
					\State \textbf{return}	
				\EndIf
				\State přidej podstrom ID3($data_{X_j=x_j}, G, X \setminus {X_j}$)
			\EndFor
		\EndFunction
	\end{algorithmic}
\end{algorithm}	
...
Pro odhad chyby bagged modelu nepotřebujeme krosvalidaci, protože bootstrapping přirozeně vytváří vhodná \uv{testovací data}. Pro každý model totiž použijeme jen cca $0.638\cdot N$ původních dat, zbytek jsou tzv. \textit{out-of-the-bag observations} = data nepoužitá v daném modelu. To znamená, že libovolný $i$-tý prvek cca $B/3$ modelů nepoužívá pro učení ($B$ = celkový počet stromů v ansámblu) -- na těchto modelech tedy můžeme testovat, jak dobře zvládnou $i$-tý prvek klasifikovat.
...
Celková predikce je tedy $\sum_{m=1}^{M} \hat{w}_m \hat{f}_m(x)$.
...
\item $lambda$ = shrinkage parametr -- zpomaluje učení (0.0001 - 0.01)
...
SVM přidávájí navíc oproti SVC ještě možnost nelineární transformace vstupního prostoru do příznakového prostoru (typicky vyšší dimenze) prostřednictvím tzv. \textbf{kernel} funkcí. Chceme tedy zobrazení $\Phi : X \mapsto F$. To může být však velmi výpočetně náročné. SVM využívají trik: v duální formulaci problému potřebujeme (jak pro konstrukci, tak pro klasifikaci) \textit{vždy pouze skalární součin dvou vektorů}, tj. $\vec{x} \cdot \vec{z}$, resp. zobrazených vektorů $\Phi(\vec{x}) \cdot \Phi(\vec{z})$. Budou nám tedy stačit funkce počítající pro dva vektory tento součin v novém prostoru, tzv. \textbf{kernel funkce}:
...
\subparagraph{Metody}
\begin{itemize}
	\item \textbf{odspoda (bottom-up}) Na začátku každý vzorek ve svém klastru. Slučuje dva klastry s minimální vzdáleností (maximal similarity) dokud nemáme 1 klastr. Možnosti měření vzdálenosti/podobnosti mezi klastry (=\textit{linkage}):
	\begin{itemize}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Comment{$\gamma_i$ je pravděpodobnost, že $z_i = 1$ v daném kroku, tj. pro aktuální parametry $\theta$. Máme tedy odhad $Z$, můžeme pokračovat k maximalizaci.}
...
\begin{itemize}
\item příklad je s hypotézou konzistentní $\to$ hypotézu \textbf{neměníme}
...
\begin{itemize}
	...
	\item Budované modely jsou typicky \textbf{robustní} vůči novým \textbf{neznámým datům} a \textbf{zašuměnému či chybnému vstupu}. Toho je u ručně psaných pravidel velmi obtížné dosáhnout.
...
\textbf{Zašuměný kanál} je označení pro model média, které k přenášeným slovům přidá nějaký šum. Formálně: do kanálu vstupuje nějaká sekvence symbolů, z něj pak vystupuje pozměněná, zašuměná sekvence, kterou my přijmeme. Cílem je tedy na základě přijaté zašuměné sekvence uhodnout původní zprávu.
...
$$A_{best} = \argmax_A \frac{p(B|A)p(A)}{p(B)} =\argmax_A p(B|A)p(A)$$
...
\begin{enumerate}
	...
	$$(k,l) = \argmax_{k,l} I_{merge(r,k,l)}(D,E)$$
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item na základě sloupce $t_{i-1}$ a aktuálního vstupního symbolu $y_i$ vytvoř následující sloupec trellis grafu, ale vyber pouze stavy, do nichž se dá dostat z aktuálního sloupce a které generují $y_{i+1}$ (resp. takové, že přechodová hrana generuje $y_{i+1}$)
...
\textbf{Parsing (parsování)} je označení pro proces, kdy je pro vstupní větu, o níž předpokládáme, že byla vytvořena nějakou gramatikou, nalezena posloupnost pravidel této gramatiky, kterou byla věta nejpravděpodobněji vytvořena. Typicky se pro větu rovnou sestrojí \textit{parsovací strom}.
...
$$\argmax_E = \argmax_E P(E|F) = P(F|E)\cdot P(E)$$
...
\textbf{Reversal $\rho(i,j)$} přetočí prvky $i$ až $j$ v $\pi$, tj. vznikne 
...
\textbf{Formální neuron} s vahami $(w_1, w_2, \dots w_n) \in \R^n$, prahem $\theta \in \R$ a přenosovou funkcí $f : \R^{n+1} \times \R^n \rightarrow \R$ počítá pro libovolný vstup $\vec{x} \in \R^n$ svůj výstup $y$ jako hodnotu přenosové funkce $f$ v $\vec{x}$, $f[\vec{w}, \theta](\vec{x})$. Mezi nejznámější \textbf{přenosové funkce} (také \textbf{aktivační funkce}) patří \textit{skoková} (unit step), \textit{sigmoidální} nebo \textit{hyperbolická tangentoida}. \textbf{Potenciál neuronu} je vážená suma jeho vstupů: $\xi = \sum\limits_{i=1}^n x_i w_i + \theta$. Právě tento potenciál je vstupem přenosové funkce (na přehledu níže je potenciál neuronu označen písmenem $z$).
...
Základním principem je natáčení vektoru $\vec{w}$ (který je kolmý na dělicí nadrovinu) ve směru kladných vzorků. Je-li výstup v~kroku 3 kladný, svírají $\vec{w}$ a $\vec{x}$ úhel menší než 90\,°, je-li záporný, je úhel větší než 90\,°. 
...
Algoritmy, které berou v úvahu i \textbf{druhou derivaci} chybové funkce, tj. nejen gradient, ale i zakřivení. Používá se \textbf{kvadratická aproximace} chybové funkce pomocí Taylorovy řady:
...
BAM lze použít pro konstrukci autoasociativních sítí, protože matice vah vytvořené Hebbovským učením jsou symetrické.
...
\begin{align*}
	\text{Neuron 1: } sgn(x_1)(0 + x_2w_{12} + x_3w_{13} + \dots + x_nw_{1n} - \theta_1) > 0\\
	\text{Neuron 2: } sgn(x_2)(x_1w_{21} + 0 + x_3w_{23} + \dots + x_nw_{2n} - \theta_2) > 0\\
	\vdots\\
	\text{Neuron n: } sgn(x_n)(x_1w_{n1} + x_2w_{n2} + x_3w_{n3} + \dots + 0 - \theta_n) > 0\\
\end{align*}
...
Což odpovídá lineární separaci $z_1,z_2,\dots z_n$. Pro $m$ vektorů $\vec{x}$ jde tedy o separaci $mn$ pomocných vektorů $z$. Pomocí perceptronového učení můžeme nalézt správný váhový vektor $\vec{v}$ (pokud separace existuje). Tento vektor kóduje váhovou matici a prahy odpovídající správnému řešení.
...
\begin{description}
	...
	\item[$k$-means ($k$ středů) -- unsupervised] Na začátku máme $k$ vektorů, každý ve svém vlastním shluku. Nový vektor je klasifikován k tomu shluku, jehož centroid je mu nejblíž. Centroid se pak aktualizuje
