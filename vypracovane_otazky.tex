...
\section{Rekurzivní a rekurzivně spočetné množiny. (TODO)}
\textit{Nekompletní. Ale další roky prý už bude otázka změněna na Rekurzivní jazyky, takže cajk.}
...
\subsubsection{Vlastnosti PRF, ORF, ČRF}
Následující platí pro PRF i ORF stejně (a často i ČRF)

\begin{theorem}[konečný součet a součin]
Je-li $f$ PRF dvou proměnných (pro jednoduchost), potom i funkce $g(z, x) = \sum_{y<z} f(y, x)$ (přičemž $g(0, x) = 0$) a $h(z, x) = \prod_{y<z} f(y, x)$ (přičemž $h(0, x) = 1$) jsou PRF.
\end{theorem}
\begin{proof}
For cyklus.
\end{proof}
...
Rozhodnutelnost, nerozhodnutelnost, halting problém s důkazem, uzavřenost na doplněk

 Postova věta, (ne)uzavřenost na operace, a pak ty vlastnost že RSM jsou oborem hodnot nějaké [rostoucí] ČRF. Doptal se mě na jiné matematické modely algoritmů, řekl jsem TS a ekvivalence RJ s RM a RSJ s RSM.
